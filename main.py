...
def operations_menu(service, logger, username):
    ...
    while True:
        ...
        print("4 - Добавить неисправность")
        print("5 - Зарегистрировать ремонт")
        print("6 - Удалить работника")
        print("7 - Изменить номер автомобиля")
        ...
        if choice == '0':
            ...
            if номер == '0':
                continue
            марка = input("Марка автомобиля: ")
            if марка == '0':
                continue
            try:
                год = int(input("Год выпуска: "))
            except ValueError:
                print("✗ Ошибка: введите числовой год")
                wait_for_continue()
                continue
            изготовитель = input("Изготовитель: ")
            if изготовитель == '0':
                continue
            владелец = input("ФИО владельца: ")
            if владелец == '0':
                continue

            try:
                result = service.add_car(номер, марка, год, изготовитель, владелец)
                logger.log(username, "ADD_CAR",
                           f"Номер={номер}, Марка={марка}, Год={год}, Владелец={владелец}")
                print(f"✓ {result['message']}")
                print(f"  Детали: {result['details']}")
            except Exception as e:
                logger.log(username, "ERROR", f"ADD_CAR failed: {str(e)}")
                print(f"✗ Ошибка: {e}")
            wait_for_continue()
